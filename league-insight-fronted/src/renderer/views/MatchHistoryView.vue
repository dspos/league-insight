...
import type { MatchHistory, Participant, GameDetail, UserTag, ChampionOption, GameModeOption, WinRate, QueueInfo } from '@/types/api'
...
const soloRank = computed((): QueueInfo | null => {
  return gameStore.soloRank
})

// 灵活组排段位
const flexRank = computed((): QueueInfo | null => {
  return gameStore.flexRank
})
...
async function loadData() {
  if (!gameStore.currentSummoner?.puuid) return

  loading.value = true
  gameDetailsCache.value = {}

  try {
    const [matches, userTagData, winRates] = await Promise.all([
      apiClient.getMatchHistory(gameStore.currentSummoner.puuid, 0, pageSize - 1),
      apiClient.getUserTagByPuuid(gameStore.currentSummoner.puuid, 0),
      apiClient.getRankedWinRates(gameStore.currentSummoner.puuid)
    ])
    matchHistory.value = matches
    userTag.value = userTagData
...
async function loadMatchHistory() {
  ...
  try {
    ...
    if (filterChampionId.value > 0 || filterQueueId.value > 0) {
      matchHistory.value = await apiClient.getFilteredMatchHistory(
      ...
      matchHistory.value = await apiClient.getMatchHistory(
        gameStore.currentSummoner.puuid,
        begIndex,
        endIndex
      )
...
// 获取该召唤师在对局中的数据
function getPlayerInMatch(match: MatchHistory): (Participant & { playerName: string }) | null {
  const puuid = gameStore.currentSummoner?.puuid
...
function isMatchWin(match: MatchHistory): boolean {
  const player = getPlayerInMatch(match)
...
function getSummonerName(): string {
  const summoner = gameStore.currentSummoner
  if (!summoner) return ''
...
function navigateToPlayer(gameName: string, tagLine: string) {
  ...
  router.push({
    path: '/summoner',
    query: { name: tagLine ? `${gameName}#${tagLine}` : gameName }
  })
}
</script>
...
<template>
  <div class="match-history-view">
    ...
    <div v-if="!gameStore.currentSummoner" class="no-data">
      <p>请先连接游戏客户端</p>
    </div>

    <div v-else-if="loading && matchHistory.length === 0" class="no-data">
      <p>加载中...</p>
    </div>
    ...
    <div v-else class="result-container">
      ...
      <div class="user-panel">
        ...
        <div class="user-card">
          <div class="user-card-header">
            <div class="avatar-wrapper">
              <img
                class="avatar-img"
                :src="`http://127.0.0.1:8080/api/v1/asset/profile/${gameStore.currentSummoner.profileIconId}`"
                alt="avatar"
              />
              <div class="level-badge">{{ gameStore.currentSummoner.summonerLevel }}</div>
            </div>
            <div class="user-info">
              <div class="user-name-row">
                <span class="user-name">{{ gameStore.currentSummoner.gameName }}</span>
                <button class="copy-btn" @click="copyName" title="复制">📋</button>
              </div>
              <div class="user-tag">
                <span>#{{ gameStore.currentSummoner.tagLine }}</span>
              </div>
            </div>
          </div>

          <!-- 标签 -->
          <div v-if="userTag?.tag?.length" class="tags-row">
            <span
        ...
        <!-- 好友/宿敌 -->
        <div v-if="userTag?.recentData?.friendAndDispute" class="relationship-section">
          ...
          <div class="relationship-col">
            ...
            <div class="relationship-list">
              <div
                v-for="dispute in userTag.recentData.friendAndDispute.disputeSummoner.slice(0, 5)"
        ...
        <div class="rank-cards">
          <div class="rank-card">
            <div class="rank-icon-area">
              ...
              <img
                class="rank-img"
                :src="getTierIcon(soloRank?.tier || '')"
                alt="tier"
              />
              <div class="rank-tier">{{ getTierCn(soloRank?.tier || '') }}</div>
              <div class="rank-division" v-if="soloRank">
                {{ getDivisionOrPoint(soloRank) }}
              </div>
            </div>
            <div class="rank-stats">
              <div class="win-rate-badge" :class="{ 'good': getWinRate(rankedWinRates?.RANKED_SOLO_5x5?.wins || 0, rankedWinRates?.RANKED_SOLO_5x5?.losses || 0) >= 58, 'bad': getWinRate(rankedWinRates?.RANKED_SOLO_5x5?.wins || 0, rankedWinRates?.RANKED_SOLO_5x5?.losses || 0) <= 49 }">
                胜率 {{ getWinRate(rankedWinRates?.RANKED_SOLO_5x5?.wins || 0, rankedWinRates?.RANKED_SOLO_5x5?.losses || 0) }}%
              </div>
              <div class="rank-wl">
                <span>胜: {{ rankedWinRates?.RANKED_SOLO_5x5?.wins || 0 }}</span>
                <span>负: {{ rankedWinRates?.RANKED_SOLO_5x5?.losses || 0 }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="rank-card">
            <div class="rank-icon-area">
              ...
              <img
                class="rank-img"
                :src="getTierIcon(flexRank?.tier || '')"
                alt="tier"
              />
              <div class="rank-tier">{{ getTierCn(flexRank?.tier || '') }}</div>
              <div class="rank-division" v-if="flexRank">
                {{ getDivisionOrPoint(flexRank) }}
              </div>
            </div>
            <div class="rank-stats">
              <div class="win-rate-badge" :class="{ 'good': getWinRate(rankedWinRates?.RANKED_FLEX_SR?.wins || 0, rankedWinRates?.RANKED_FLEX_SR?.losses || 0) >= 58, 'bad': getWinRate(rankedWinRates?.RANKED_FLEX_SR?.wins || 0, rankedWinRates?.RANKED_FLEX_SR?.losses || 0) <= 49 }">
                胜率 {{ getWinRate(rankedWinRates?.RANKED_FLEX_SR?.wins || 0, rankedWinRates?.RANKED_FLEX_SR?.losses || 0) }}%
              </div>
              <div class="rank-wl">
                <span>胜: {{ rankedWinRates?.RANKED_FLEX_SR?.wins || 0 }}</span>
                <span>负: {{ rankedWinRates?.RANKED_FLEX_SR?.losses || 0 }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- 最近表现 -->
        <div v-if="userTag?.recentData" class="recent-stats-card">
          ...
          <div class="stat-row">
            ...
            <div class="stat-value-group">
              <span class="stat-kda-main" :style="{ color: getKdaColor(userTag.recentData.kda) }">
                {{ userTag.recentData.kda.toFixed(1) }}
              </span>
              <span class="stat-detail">
                <span :style="{ color: getKillsColor(userTag.recentData.kills) }">{{ userTag.recentData.kills.toFixed(1) }}</span>
                /
                <span :style="{ color: getDeathsColor(userTag.recentData.deaths) }">{{ userTag.recentData.deaths.toFixed(1) }}</span>
                /
                <span :style="{ color: getAssistsColor(userTag.recentData.assists) }">{{ userTag.recentData.assists.toFixed(1) }}</span>
              </span>
            </div>
          </div>
          ...
          <div class="stat-row">
            ...
            <div class="stat-value-group">
              <div class="progress-bar">
                <div class="progress-fill" :style="{ width: getWinRate(userTag.recentData.selectWins, userTag.recentData.selectLosses) + '%', backgroundColor: getWinRateColor(getWinRate(userTag.recentData.selectWins, userTag.recentData.selectLosses)) }"></div>
              </div>
              <span class="stat-value" :style="{ color: getWinRateColor(getWinRate(userTag.recentData.selectWins, userTag.recentData.selectLosses)) }">
                {{ getWinRate(userTag.recentData.selectWins, userTag.recentData.selectLosses) }}%
              </span>
            </div>
          </div>
          ...
          <div class="stat-row">
            ...
            <div class="stat-value-group">
              <div class="progress-bar">
                <div class="progress-fill" :style="{ width: (userTag.recentData.groupRate || 0) + '%' }"></div>
              </div>
              <span class="stat-value">{{ userTag.recentData.groupRate || 0 }}%</span>
            </div>
          </div>

          <div class="stat-row">
            <span class="stat-label">伤害</span>
            <span class="stat-raw">{{ formatNumber(userTag.recentData.averageDamageDealtToChampions) }}</span>
            <div class="progress-bar small">
              <div class="progress-fill" :style="{ width: (userTag.recentData.damageDealtToChampionsRate || 0) + '%' }"></div>
            </div>
            <span class="stat-value">{{ userTag.recentData.damageDealtToChampionsRate || 0 }}%</span>
          </div>

          <div class="stat-row">
            <span class="stat-label">经济</span>
            <span class="stat-raw">{{ formatNumber(userTag.recentData.averageGold) }}</span>
            <div class="progress-bar small">
              <div class="progress-fill" :style="{ width: (userTag.recentData.goldRate || 0) + '%' }"></div>
            </div>
            <span class="stat-value">{{ userTag.recentData.goldRate || 0 }}%</span>
          </div>
        </div>
      </div>
      ...
      <div class="match-panel">
        ...
        <div v-if="matchHistory.length === 0" class="no-matches">
          <p>暂无战绩数据</p>
        </div>
        ...
        <div v-else class="match-list">
          <div
            ...
            :key="match.gameId"
            class="match-card"
            :class="{ win: isMatchWin(match), loss: !isMatchWin(match) }"
            @click="showMatchDetail(match)"
          >
            <!-- 卡片头部：胜负/模式/时间 -->
            <div class="match-card-header" :class="{ win: isMatchWin(match), loss: !isMatchWin(match) }">
              <div class="match-header-left">
                <span class="match-result-tag">{{ isMatchWin(match) ? '胜利' : '失败' }}</span>
            ...
            <div class="match-card-body">
              ...
              <div class="match-hero-section">
                <div class="match-hero-avatar">
                  <img
                    class="hero-img"
                    :src="getChampionUrl(getPlayerInMatch(match)?.championId)"
                    alt="champion"
                    @error="handleImageError"
                  />
                  <div
                    v-if="getMvpText(getPlayerInMatch(match)?.stats)"
                    class="mvp-tag"
                    :class="{ mvp: getMvpText(getPlayerInMatch(match)?.stats) === 'MVP', svp: getMvpText(getPlayerInMatch(match)?.stats) === 'SVP' }"
                  >
                    {{ getMvpText(getPlayerInMatch(match)?.stats) }}
                  </div>
                </div>
                <div class="match-hero-stats">
                  <div class="kda-display">
                    <span class="kda-num" :style="{ color: getKillsColor(getPlayerInMatch(match)?.stats?.kills || 0) }">
                      {{ getPlayerInMatch(match)?.stats?.kills }}
                    </span>
                    <span class="kda-sep">/</span>
                    <span class="kda-num" :style="{ color: getDeathsColor(getPlayerInMatch(match)?.stats?.deaths || 0) }">
                      {{ getPlayerInMatch(match)?.stats?.deaths }}
                    </span>
                    <span class="kda-sep">/</span>
                    <span class="kda-num" :style="{ color: getAssistsColor(getPlayerInMatch(match)?.stats?.assists || 0) }">
                      {{ getPlayerInMatch(match)?.stats?.assists }}
                    </span>
                    <span class="kda-ratio-text" :style="{ color: getKdaColor(getKdaValue(match)) }">
                      {{ getKdaValue(match).toFixed(1) }}
                    </span>
                  </div>
                  <div class="match-gold-cs">
                    <span>{{ formatNumber(getPlayerInMatch(match)?.stats?.goldEarned) }} 金币</span>
                    <span>{{ (getPlayerInMatch(match)?.stats?.neutralMinionsKilled || 0) + (getPlayerInMatch(match)?.stats?.minionsKilled || 0) }} 补兵</span>
                  </div>
                </div>
              </div>
              ...
              <div class="match-build-section">
                <div class="build-top-row">
                  <div class="spells-mini">
                    <img
                      v-if="getPlayerInMatch(match)?.spell1Id"
                      class="spell-mini-img"
                      :src="getSpellUrl(getPlayerInMatch(match)?.spell1Id)"
                      @error="handleImageError"
                    />
                    <img
                      v-if="getPlayerInMatch(match)?.spell2Id"
                      class="spell-mini-img"
                      :src="getSpellUrl(getPlayerInMatch(match)?.spell2Id)"
                      @error="handleImageError"
                    />
                  </div>
                  <div class="runes-mini">
                    <img
                      v-if="getPlayerInMatch(match)?.stats?.perk0"
                      class="rune-mini-img"
                      :src="`http://127.0.0.1:8080/api/v1/asset/perk/${getPlayerInMatch(match)?.stats?.perk0}`"
                      @error="handleImageError"
                    />
                  </div>
                </div>
                <div class="items-mini">
                  <img
                    v-for="(itemId, idx) in getItemIds(getPlayerInMatch(match)?.stats)"
                    :key="idx"
                    class="item-mini-img"
              ...
              <div class="match-damage-section">
                <div class="damage-stat">
                  ...
                  <span class="damage-value" :style="{ color: getOtherColor(getPlayerInMatch(match)?.stats?.damageDealtToChampionsRate || 0) }">
                    {{ formatNumber(getPlayerInMatch(match)?.stats?.totalDamageDealtToChampions) }}
                  </span>
                  <div class="damage-bar">
                    <div class="damage-fill damage" :style="{ width: Math.min(100, (getPlayerInMatch(match)?.stats?.damageDealtToChampionsRate || 0)) + '%' }"></div>
                  </div>
                </div>
                <div class="damage-stat">
                  <span class="damage-label">承伤</span>
                  <span class="damage-value" :style="{ color: getHealColor(getPlayerInMatch(match)?.stats?.damageTakenRate || 0) }">
                    {{ formatNumber(getPlayerInMatch(match)?.stats?.totalDamageTaken) }}
                  </span>
                  <div class="damage-bar">
                    <div class="damage-fill taken" :style="{ width: Math.min(100, (getPlayerInMatch(match)?.stats?.damageTakenRate || 0)) + '%' }"></div>
                  </div>
                </div>
              </div>
              ...
              <div class="match-teams-section" v-if="gameDetailsCache[match.gameId]">
                <div class="team-mini-row">
                  <div
                    ...
                    :class="{ 'is-me': player.puuid === gameStore.currentSummoner?.puuid }"
    ...
    <MatchDetailModal
      ...
      :current-puuid="gameStore.currentSummoner?.puuid || ''"
      :current-summoner-name="getSummonerName()"
