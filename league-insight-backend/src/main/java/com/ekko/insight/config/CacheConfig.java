...
public class CacheConfig {
     ...
     * 缓存配置项
     */
    public record CacheSpec(String name, long maxSize, long expireAfterWrite, TimeUnit timeUnit) {
    }

    private static final CacheSpec[] DEFAULT_SPECS = {
            new CacheSpec("summoner", 500, 10, TimeUnit.MINUTES),
            new CacheSpec("rank", 500, 5, TimeUnit.MINUTES),
            new CacheSpec("matchHistory", 200, 10, TimeUnit.MINUTES),
            new CacheSpec("gameDetail", 500, 30, TimeUnit.MINUTES),
            new CacheSpec("aiAnalysis", 100, 30, TimeUnit.MINUTES),
            new CacheSpec("userTag", 300, 15, TimeUnit.MINUTES),
            new CacheSpec("aramBalance", 100, 60, TimeUnit.MINUTES),
    };
