...
import com.ekko.insight.aspect.PerformanceMonitorAspect;
import com.ekko.insight.config.CacheConfig;
...
public class PerformanceController {
    ...
    public ApiResponse<List<CachePerformance>> getCachePerformance() {
        ...
        List<CachePerformance> result = stats.entrySet().stream()
                .map(entry -> new CachePerformance(
                        entry.getValue().name(),
                        entry.getValue().estimatedSize(),
                        entry.getValue().hitCount(),
                        entry.getValue().missCount(),
                        entry.getValue().hitRate(),
                        entry.getValue().avgLoadTimeMillis(),
