...
echo [4/4] 构建完成
echo 输出文件: target\league-insight-native.exe
echo.
...
echo.
echo 运行测试:
echo   .\target\league-insight-native.exe
echo.
...
```bash
# 运行时可调整参数
league-insight-native.exe -Xmx512m -Xms128m

# 启用 JIT 编译器优化（默认已启用）
league-insight-native.exe -XX:+UseParallelGC
```
